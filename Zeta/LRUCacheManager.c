...
void Zeta_LRUCacheManager_Init(void* lrucm_) {
    ...
    Zeta_OrdDoublyLinkedNode_Init(&lrucm->ln_list_head);
}

Zeta_LRUCacheManager_Node* FindWithKey_(Zeta_LRUCacheManager* lrucm,
                                        size_t key) {
    Zeta_OrdDoublyLinkedNode* ln = &lrucm->ln_list_head;

    for (;;) {
        ln = Zeta_OrdDoublyLinkedNode_GetR(ln);
        if (ln == &lrucm->ln_list_head) { return NULL; }

        Zeta_LRUCacheManager_Node* node =
            ZETA_GetStructFromMem(Zeta_LRUCacheManager_Node, ln, ln);
...
Zeta_CacheManager_Node* Zeta_LRUCacheManager_Find(void* lrucm_, size_t key) {
    ...
    Zeta_OrdDoublyLinkedNode_Extract(&node->ln);
    Zeta_OrdDoublyLinkedNode_InsertR(&lrucm->ln_list_head, &node->ln);
...
Zeta_CacheManager_Node* Zeta_LRUCacheManager_Push(void* lrucm_, size_t key) {
    ...
    if (node == NULL) {
        ...
        Zeta_OrdDoublyLinkedNode_Init(&node->ln);
        node->cn.key = key;
        node->cn.frame = NULL;
        node->cn.flags = 0;
    } else {
...
Zeta_CacheManager_Node Zeta_LRUCacheManager_Pop(void* lrucm_) {
    ...
    Zeta_OrdDoublyLinkedNode* target_ln =
        Zeta_OrdDoublyLinkedNode_GetL(&lrucm->ln_list_head);
    ...
    Zeta_LRUCacheManager_Node* target_node =
        ZETA_GetStructFromMem(Zeta_LRUCacheManager_Node, ln, target_ln);

    Zeta_OrdDoublyLinkedNode_Extract(target_ln);
...
Zeta_CacheManager_Node Zeta_LRUCacheManager_PopWithKey(void* lrucm_,
    ...
    Zeta_OrdDoublyLinkedNode_Extract(&target_node->ln);
