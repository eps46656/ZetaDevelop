...
ZETA_StaticAssert(ZETA_MultiLevelTable_max_branch_num <= ZETA_WidthOf(uN_t));
...
static Zeta_MultiLevelTable_Node* AllocateNode_(
    ...
    ZETA_DebugAssert(
        ZETA_PtrToAddr(node_) % alignof(Zeta_MultiLevelTable_Node) == 0);
...
void Zeta_MultiLevelTable_Check(void* mlt_, Zeta_DebugHashTable* dst_node) {
    Zeta_MultiLevelTable* mlt = mlt_;
    ZETA_DebugAssert(mlt != NULL);
