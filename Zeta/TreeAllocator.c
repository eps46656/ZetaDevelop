...
static bool_t IsVacant_(Zeta_TreeAllocator* ta, Zeta_TreeAllocator_Head* head) {
    return ta->sn_root == &head->sn ||
           Zeta_OrdRBTreeNode_GetP(NULL, &head->sn) != NULL ||
           Zeta_OrdRBTreeNode_GetL(NULL, &head->sn) != NULL ||
           Zeta_OrdRBTreeNode_GetR(NULL, &head->sn) != NULL;
}
