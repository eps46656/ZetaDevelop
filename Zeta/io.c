...
static void IntToStr_(Zeta_Pipe* dst, unsigned long long val,
    ...
    ZETA_DebugAssert(sign_char == '\n' || sign_char == '-' || sign_char == '+');

    void* dst_context = dst->context;

    void (*Write)(void* context, size_t cnt, void const* data) = dst->Write;
    ...
    if (!l_just) {
        ...
        for (size_t i = sign_width + digit_width; i < just_width; ++i) {
            Write(dst_context, 1, &just_char);
        }
    }

    if (sign_char == '-' || sign_char == '+') {
        Write(dst_context, 1, &sign_char);
        ++cur_width;
    }

    if (val == 0) {
        Write(dst_context, 1, chars);
        ++cur_width;
    } else {
        for (; 0 < val; val /= base, ++cur_width) {
            unsigned long long k = val % base;
            Write(dst_context, 1, chars + k);
        }
    }
...
void Zeta_SIntToStr(Zeta_Pipe* dst, long long val, bool_t sign,
                    unsigned long long base, bool_t l_just, size_t just_width,
                    unichar_t just_char) {
    IntToStr_(dst, val < 0 ? -(unsigned long long)val : val,
              val < 0 ? '-' : (sign ? '+' : '\0'), base, lower_chars, l_just,
              just_width, just_char);
}
...
void Zeta_SIntToHexStr(Zeta_Pipe* dst, long long val, bool_t sign,
