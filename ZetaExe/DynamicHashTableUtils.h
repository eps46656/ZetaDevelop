...
void DynamicHashTable_Init(
    Zeta_AssocContainer* assoc_cntr, void* elem_hash_context,
    ...
    Zeta_DynamicHashTable_DeployAssocContainer(&pack->dht, assoc_cntr);

    AssocContainer_AddSanitizeFunc(Zeta_DynamicHashTable_GetWidth,
                                   DynamicHashTable_Sanitize);
}

void DynamicHashTable_Deinit(Zeta_AssocContainer* assoc_cntr) {
    if (assoc_cntr == NULL ||
...
template <typename Val>
Zeta_AssocContainer* DynamicHashTable_Create(
    ...
    int (*ElemKeyCompare)(void* elem_key_cmp_context, void const* elem,
                          void const* key)) {
    Zeta_AssocContainer* assoc_cntr{ new Zeta_AssocContainer{} };
...
void DynamicHashTable_Destroy(Zeta_AssocContainer* assoc_cntr) {
    if (assoc_cntr == NULL ||
        assoc_cntr->GetSize != Zeta_DynamicHashTable_GetSize) {
        return;
    }
...
void DynamicHashTable_Sanitize(Zeta_AssocContainer const* assoc_cntr) {
